...
elseif ($orderedby == "ewallet")
{
$orderedbyq = "ewallet";
}
elseif ($orderedby == "credits")
{
$orderedbyq = "credits";
}
...
$newmemberq = "insert into members (userid,password,accounttype,membershiplastpaid,firstname,lastname,country,email,egopay,payza,perfectmoney,okpay,solidtrustpay,paypal,signupdate,signupip,referid,lastsolopost,nextsolopost,verifycode) values (\"$new_userid\",\"$new_password\",\"$new_accounttype\",\"$today\",\"$new_firstname\",\"$new_lastname\",\"$new_country\",\"$new_email\",\"$new_egopay\",\"$new_payza\",\"$new_perfectmoney\",\"$new_okpay\",\"$new_solidtrustpay\",\"$new_paypal\",NOW(),\"$new_signupip\",\"$newref\",NOW(),NOW(),\"$unixtimestamp\")";
$newmemberr = mysql_query($newmemberq) or die(mysql_error());
			...
			";
			@mail($toadmin, $subjectadmin, wordwrap(stripslashes($messageadmin)),$headersadmin, "-f$adminemail");
			$bonuslevel = "bonusfree";
			} # if ($new_accounttype != "PAID")

#### START SIGNUP BONUSES ####
$bonusq = "select * from bonuses where enable=\"yes\" and $bonuslevel=\"yes\" and bonustype=\"Sign-Up\" order by id";
...
if ($bonusrows > 0)
	...
	while ($bonusrowz = mysql_fetch_array($bonusr))
		...
		$credits = $bonusrowz["credits"];
		$solo_num = $bonusrowz["solo_num"];
		$banner_num = $bonusrowz["banner_num"];
		$banner_views = $bonusrowz["banner_views"];
		$button_num = $bonusrowz["button_num"];
		$button_views = $bonusrowz["button_views"];
		$textad_num = $bonusrowz["textad_num"];
		$textad_views = $bonusrowz["textad_views"];
		$fullloginad_num = $bonusrowz["fullloginad_num"];
		$fullloginad_views = $bonusrowz["fullloginad_views"];
		...
		mysql_query("insert into transactions (userid,transaction,description,paymentdate,amountreceived) VALUES ('".$userid."','New $new_accounttype Member Bonus','" . $bonusname . " " . $bonustype . " Bonus',NOW(),'0.00')");
		} # while ($bonusrowz = mysql_fetch_array($bonusr))
	} # if ($bonusrows > 0)
#### END SIGNUP BONUSES ####
...
if ($rows > 0)
{
mysql_query("delete from banners where userid='$delete_userid'");
mysql_query("delete from bannerviews where userid='$delete_userid'");
mysql_query("delete from banners_saved where userid='$delete_userid'");
mysql_query("delete from bounces where userid='$delete_userid'");
mysql_query("delete from buttons where userid='$delete_userid'");
mysql_query("delete from buttonviews where userid='$delete_userid'");
mysql_query("delete from buttons_saved where userid='$delete_userid'");
mysql_query("delete from creditsolos where userid='$delete_userid'");
mysql_query("delete from creditsolos_saved where userid='$delete_userid'");
mysql_query("delete from creditsolos_viewed where userid='$delete_userid'");
mysql_query("delete from cashoutrequests where userid='$delete_userid'");
mysql_query("delete from downlinemails where userid='$delete_userid'");
mysql_query("delete from textads where userid='$delete_userid'");
mysql_query("delete from textadviews where userid='$delete_userid'");
mysql_query("delete from textads_saved where userid='$delete_userid'");
mysql_query("delete from fullloginads where userid='$delete_userid'");
mysql_query("delete from fullloginadviews where userid='$delete_userid'");
mysql_query("delete from fullloginads_saved where userid='$delete_userid'");
mysql_query("delete from offerpages_viewed where userid='$delete_userid'");
mysql_query("delete from payouts where userid='$delete_userid'");
mysql_query("delete from promocodes_used where userid='$delete_userid'");
mysql_query("delete from adminemails_viewed where userid='$delete_userid'");
mysql_query("delete from solos where userid='$delete_userid'");
mysql_query("delete from solos_saved where userid='$delete_userid'");
mysql_query("delete from solos_viewed where userid='$delete_userid'");
mysql_query("delete from support where userid='$delete_userid'");
mysql_query("delete from textads where userid='$delete_userid'");
mysql_query("delete from textads_saved where userid='$delete_userid'");
mysql_query("delete from textadviews where userid='$delete_userid'");
...
$givecommissions = $_POST["givecommissions"];
$deductcost = $_POST["deductcost"];
...
$savememberq = "update members set password='$password',accounttype='$accounttype',membershiplastpaid='$membershiplastpaid',firstname='$firstname',lastname='$lastname',country='$country',email='$email',egopay='$egopay',payza='$payza',perfectmoney='$perfectmoney',okpay='$okpay',solidtrustpay='$solidtrustpay',paypal='$paypal',signupip='$signupip',referid='$referid',ewallet='$ewallet',vacation='$vacation',verified='$verified' where id='$saveid'";
$savememberr = mysql_query($savememberq);

if (($accounttype == "FREE") and ($oldaccounttype == "PAID"))
	{
		#### START SIGNUP BONUSES ####
		$bonusq = "select * from bonuses where enable=\"yes\" and bonusfree=\"yes\" and bonustype=\"Sign-Up\" order by id";
		$bonusr = mysql_query($bonusq);
		$bonusrows = mysql_num_rows($bonusr);
		if ($bonusrows > 0)
			{
			while ($bonusrowz = mysql_fetch_array($bonusr))
				...
				$bonusq2 = "update bonuses set howmanytimesgiven=howmanytimesgiven+1 where id=\"$bonusid\"";
				$bonusr2 = mysql_query($bonusq2);
				mysql_query("insert into transactions (userid,transaction,description,paymentdate,amountreceived) VALUES ('".$userid."','New FREE Member Bonus','" . $bonusname . " " . $bonustype . " Bonus',NOW(),'0.00')");
				} # while ($bonusrowz = mysql_fetch_array($bonusr))
			} # if ($bonusrows > 0)
		#### END SIGNUP BONUSES ####
	} # if (($accounttype == "FREE") and ($oldaccounttype == "PAID"))
...
if (($accounttype == "PAID") and ($oldaccounttype == "FREE"))
	...
	if ($deductcost == "yes")
		{
		$eq = "update members set ewallet=ewallet-" . $joinprice . ",membershiplastpaid=\"$today\" where userid=\"$userid\"";
		$er = mysql_query($eq);
		mysql_query("INSERT INTO transactions (userid,transaction,description,paymentdate,amountreceived) VALUES ('".$userid."','".$ewalletname."','".$ewalletname." Purchase - Member Paid Upgrade - Upgraded from Admin area',NOW(),'".$joinprice."')");
		}
	if ($givecommissions == "yes")
		...
		# sponsor commission
			...
			if ($refrows > 0)
				...
				$ewalletq = "update members set ewallet=ewallet+" . $adpackcommission . " where userid=\"$saveref\"";
				$ewalletr = mysql_query($ewalletq);
				$refq3 = "insert into payouts (userid,paid,datepaid,description) values (\"$saveref\",\"$adpackcommission\",NOW(),\"$ewalletname Sponsor Payment - Referral $userid Upgraded by Admin\")";
...
if ($vacation != $oldvacation)
	{
	$vacq = "update members set vacationdate=DATE_SUB(NOW(),INTERVAL 24 HOUR) where userid=\"$userid\"";
	$vacr = mysql_query($vacq);
	}
...
<option value="ewallet">E-Wallet Balance</option>
<?php
if ($enablecreditssystem == "yes")
{
?>
...
$vacation = $rowz["vacation"];
$vacationdate = $rowz["vacationdate"];
if ($vacation == "yes")
{
$bgvacation = "#ff9999";
$showvacation = "YES";
...
{
?>
<td align="center"><input type="text" name="membershiplastpaid" value="N/A" size="10" maxlength="255" class="typein"></td>
<?php
}
if ($accounttype == "PAID")
{
...
<tr class="sabrinatrlight"><td><a href="membercontrol.php?orderedby=vacation&searchfor=<?php echo $searchfor ?>&searchby=<?php echo $searchby ?>">Vacation</a>:</td>
<td>
<select name="vacation" style="background-color:<?php echo $bgvacation ?>">
<option value="yes" <?php if ($vacation == "yes") { echo "selected"; } ?>>YES</option>
<option value="no" <?php if ($vacation != "yes") { echo "selected"; } ?>>NO</option>
</select>
</td></tr>
<tr class="sabrinatrlight"><td><a href="membercontrol.php?orderedby=ewallet&searchfor=<?php echo $searchfor ?>&searchby=<?php echo $searchby ?>">E-Wallet Balance</a>:</td><td><input type="text" name="ewallet" value="<?php echo $ewallet ?>" size="6" maxlength="12" class="typein"></td></tr>
<tr class="sabrinatrlight"><td><a href="membercontrol.php?orderedby=credits&searchfor=<?php echo $searchfor ?>&searchby=<?php echo $searchby ?>">Credits Balance</a>:</td><td><input type="text" name="credits" value="<?php echo $credits ?>" size="6" maxlength="12" class="typein"></td></tr>
...
<tr class="sabrinatrlight"><td>Deduct cost of paid membership ($<?php echo $joinprice ?> if upgrading from <?php echo $level1name ?> to <?php echo $level2name ?> membership:</td><td valign="top"><input type="checkbox" name="deductcost" value="yes" checked></td></tr>
<?php
...
if ($adpackrows > 0)
	{
	?>
	<tr class="sabrinatrlight"><td>Bonus AdPack if upgrading from <?php echo $level1name ?> to <?php echo $level2name ?> membership:</td>
	<td>
	<select name="adpackid">
	<option value="NONE">No Bonus AdPack</option>
		<?php
